---
name: Mobile game art delivery
description: Why game-critical images must be bundled into the Expo app, and two rendering traps — RN Image never retries failed loads, and RNW sizes bundled images intrinsically.
---

**Rule 1: Bundle game-critical art into the app (`require()`), don't fetch it from the api-server.**
...
**How to apply:** Anything rendered during core gameplay ships in `assets/game/` with a `require()` map (see `lib/gameArt.ts`, codegen'd `game/battleViewSources.ts`). Menu/decor art may stay remote — it loads singly and degrades gracefully. If remote art must burst-load, the storage route needs a high limiter ceiling + `Cache-Control: public, max-age=31536000, immutable` (it serves immutable objects), and the global limiter must not stack on top of the storage route.
