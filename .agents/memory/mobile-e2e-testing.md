...
- STALE (as of Jul 2026): the battle modal no longer auto-dismisses at all — see [Battle dice-turn pacing](battle-dice-pacing.md). Every dice turn and the final result require an explicit tap; a tester must click through and check on-screen state directly, not infer completion from a vanished scene.
