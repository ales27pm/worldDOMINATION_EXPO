...
import { generateMissions, missionAchieved } from "./missions";
import type {
  AllianceLevel, ContinentId, ElectionState, GameAction, GameSetup, GameState,
  LogTone, PlayerState, TerritoryId, TerritoryState,
} from "./types";
...
function drawCard(state: GameState, player: PlayerState): void {
  if (state.deck.length === 0) state.deck = buildDeck(state.activeIds);
  const card = state.deck.pop();
  if (card) player.cards = [...player.cards, card];
}
...
function checkVictory(state: GameState, playerId: number): void {
  ...
  if (aliveOthers.length === 0) { declare("total conquest — every rival destroyed"); return; }
  ...
  if (objective === "domination60" || objective === "domination80") {
    ...
    if (owned >= target) { declare(`${objective === "domination60" ? "60%" : "80%"} Domination — ${owned} of ${total} territories`); return; }
  }
  if (objective === "domination100" && owned === total) { declare("World Domination"); return; }
  if (objective === "capital") {
    ...
    const captured = opposing.filter((p) => p.capital !== null && state.territories[p.capital].owner === playerId).length;
    if (own !== null && required > 0 && state.territories[own].owner === playerId && captured >= required) {
      declare(`Capital RISK — ${captured} enemy capitals seized`); return;
    }
  }
...
export function createGame(setup: GameSetup): GameState {
  ...
  if (allocation === "grab" || allocation === "election") {
    ...
    dealt.forEach((id, i) => { territories[id] = { owner: i % players.length, armies: 1 }; });
  ...
  if (setup.objective === "mission") {
    ...
    players.forEach((p, i) => { p.mission = missions[i] ?? null; });
  }

  const state: GameState = {
    setup, players, territories, activeIds,
    deck: buildDeck(activeIds),
    currentPlayer: 0, phase: "reinforcement", turn: 1,
    reinforcementsRemaining: 0, mustTrade: false, tradesCompleted: 0,
    pendingOccupy: null, lastBattle: null, winner: null, winReason: null,
    battlesFought: 0, awaitingHandoff: false, log: [], logCounter: 0,
    alliances: [], pendingProposal: null, proposalsMade: [],
    initialRemaining: players.map(() => initial),
    history: [], election: null,
  };
  addLog(state, `The campaign of World Domination begins — ${OBJECTIVE_INFO[setup.objective].name}.`, "gold");
  if (allocation === "grab") {
...
function beginElections(state: GameState): void {
  ...
  const perPlayer = Math.floor((state.activeIds.length * POINTS_PER_TERRITORY) / state.players.length / 10) * 10;
  const [first, ...queue] = open;
  if (!first) { beginCampaign(state); return; }
  state.phase = "election";
  state.currentPlayer = 0;
  state.election = { territory: first, queue, bid: 0, highBidder: null, passed: [], points: state.players.map(() => perPlayer), influenceUsed: [] };
  addLog(state, `The elections begin — each commander holds ${perPlayer} Election Points.`, "gold");
}
...
export function electionBudget(state: GameState, playerId: number): number {
  ...
  return (election.points[playerId] ?? 0) + influenceSources(state, playerId, election.territory).length * INFLUENCE_VALUE;
}
...
function resolveAuction(state: GameState): void {
  ...
  addLog(state, `${state.players[winner]?.name ?? "?"} wins ${name} at ${election.bid} points.`, "gold");
...
function nextAuction(state: GameState, lastWinner: number): void {
  ...
  if (!next) { finishElections(state); return; }
...
function beginCampaign(state: GameState): void {
  ...
  const firstChooser = state.setup.objective === "capital"
    ? state.players.findIndex((p) => p.isHuman && p.capital === null) : -1;
  if (firstChooser >= 0) {
...
function endTurn(state: GameState): void {
  ...
  for (let step = 1; step <= state.players.length; step += 1) {
    ...
    if (candidatePlayer?.alive) {
      next = candidate;
      if (candidate <= state.currentPlayer) { state.turn += 1; wrapped = true; }
      break;
    }
  }
  if (wrapped) {
    ...
    if (lapsed.length > 0) {
      ...
      for (const a of lapsed) {
        addLog(state, `The pact between ${state.players[a.a]?.name ?? "?"} and ${state.players[a.b]?.name ?? "?"} has lapsed.`, "info");
      }
    }
  }
...
function performTrade(state: GameState, cardIds: string[]): void {
  ...
  const cards = cardIds.map((id) => player.cards.find((c) => c.id === id)).filter((c): c is NonNullable<typeof c> => c !== undefined);
...
function forgeAlliance(state: GameState, a: number, b: number, level: AllianceLevel): void {
  ...
  addLog(state, `${state.players[a]?.name ?? "?"} and ${state.players[b]?.name ?? "?"} forge a ${ALLIANCE_LEVEL_INFO[level].name}.`, "gold");
}
  ...
  switch (action.type) {
    ...
    case "PROPOSE_ALLIANCE": {
      ...
      if (target.isHuman) {
        ...
        addLog(state, `${target.name} declines ${player.name}'s offer.`, "crimson");
      }
      return state;
    }
    case "SEND_THREAT": {
      ...
      const target = state.players[action.target];
      if (!target || !target.alive || target.isHuman) return previous;
      if (state.proposalsMade.includes(target.id)) return previous;
      state.proposalsMade = [...state.proposalsMade, target.id];
      bumpGrudge(state, target.id, player.id, 0.5);
      addLog(state, `${player.name} sends a menacing dispatch to ${target.name}.`, "crimson");
      return state;
    }
    case "RESPOND_PROPOSAL": {
      ...
      if (action.accept) {
        ...
        addLog(state, `${state.players[proposal.to]?.name ?? "?"} rebuffs the offer.`, "info");
      }
      return state;
    }
    case "CHOOSE_CAPITAL": {
    ...
    case "ATTACK": {
      ...
      // Validate adjacency
      const activeSetAtk = new Set(state.activeIds);
      const fromNeighborsAtk = (TERRITORY_MAP[action.from]?.neighbors ?? []).filter((n) => activeSetAtk.has(n));
      if (!fromNeighborsAtk.includes(action.to)) return previous;
      ...
      setTerritory(state, action.to, { owner: conquered ? player.id : to.owner, armies: defenderArmies });
      state.lastBattle = {
        from: action.from, to: action.to, attacker: player.id, defender: defenderPlayer.id,
        attackerRolls: lastAttackRolls, defenderRolls: lastDefendRolls,
        attackerLosses, defenderLosses, rounds, conquered,
      ...
      if (conquered) {
        ...
        addLog(state, `${player.name} storms ${action.to} from ${action.from} (${defenderLosses} defenders slain).`, "battle");
        const defenderRemaining = state.activeIds.filter((id) => state.territories[id].owner === defenderPlayer.id).length;
        ...
        addLog(state, `Battle at ${action.to}: attacker loses ${attackerLosses}, defender loses ${defenderLosses}.`, "battle");
      }
      return state;
    }
    case "OCCUPY": {
    ...
    case "FORTIFY": {
      ...
      // Validate adjacency
      const activeSetFrt = new Set(state.activeIds);
      const fromNeighborsFrt = (TERRITORY_MAP[action.from]?.neighbors ?? []).filter((n) => activeSetFrt.has(n));
      if (!fromNeighborsFrt.includes(action.to)) return previous;
